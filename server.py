...
def server_run(port, adr):
    ...
    s.bind(('', port))
    s.listen()
    logger_server.debug(f'Сервер запущен и находится в режиме прослушиваия.')
    ...
    try:
        while True:
            ...
            if adr != DEF_ADR_CLIENT and re.match(adr, client_adr[0]) is None:
                ...
                client.close()
                logger_server.debug(f'Закрыта сессия с клиентом.')
                return None
            ...
            logger_server.debug(f'Сервер обработал сообщение.')

            data_out = json.dumps(server_msg)
            client.send(data_out.encode(CODE))
            logger_server.debug(f'Сервер отправил ответ.')
        ...
        s.close()
        logger_server.debug(f'Сокет закрыт.')
